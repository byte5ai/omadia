...
This directory holds plain-JSON translations for the web-dev UI. **No external service** (no i18nexus, no Crowdin) — translations are reviewed and merged via normal pull-request flow.
...
Locale is picked per request in this order (`web-dev/i18n/request.ts`):
...
3. **`DEFAULT_LOCALE`** in `web-dev/i18n/locales.ts` — final fallback, currently `'en'`.
...
1. Add the language code to `LOCALES` in `web-dev/i18n/locales.ts` and to `LOCALE_LABELS`.
2. Copy `en.json` to `messages/<locale>.json` and translate values in place.
3. Add the locale to `TARGET_LOCALES` in `web-dev/scripts/i18n-validate.mjs`.

...
- `docs/CHANGELOG.md` is updated automatically in lockstep with every tagged
  release (categorized Added/Changed/Fixed sections, generated by
  `.github/scripts/generate-changelog.mjs`) via a `docs(changelog)` PR that
  auto-merges once CI is green. **Do not hand-edit past version sections** —
  they'll be regenerated the same way if the file is ever reset. You *can*
  still add hand-written notes under `## [Unreleased]` while you work; they're
  carried over verbatim into the next version's entry, above the generated
  bullet list.

...
export function Workspace({ initialDraft }: WorkspaceProps): React.ReactElement {
  ...
  // Template-Switcher gating (B.6-9 / Open Question #3). Allowed when the
  // switch would be lossless: only slots / tools / depends_on are
  // template-shape-specific, so as long as those three are empty, the
  // operator can switch without losing work. Identity (id, name,
  // description, skill, playbook, setup_fields, network.outbound) is
  // template-agnostic and survives the switch unchanged. The earlier
  // stricter heuristic (also gated on transcript + spec.id) blocked
  // operators who had only chatted with the Builder a few turns without
  // committing to a shape — that's exactly the moment a switch is most
  // valuable.
  const isDraftSwitchable =
    Object.keys(draft.slots).length === 0 &&
    draft.spec.tools.length === 0 &&
    draft.spec.depends_on.length === 0;
...
function WorkspaceHeader({
  ...
  // Marcel-Feedback 2026-05-06: Model-Switch ist eine reine Metadaten-

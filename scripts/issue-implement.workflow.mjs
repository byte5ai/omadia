...
function fixupPrompt(group, branch, findings) {
  ...
  return [
    ...
    '- Normally: add a commit on top. Do not amend or rebase.',
    '- EXCEPTION: if a finding is about the commits themselves (subject too long, wrong prefix), you may',
    '   reword them -- but ONLY after `git ls-remote --heads origin ' + branch + '` returns nothing, proving',
    '   nobody has ever seen this history. Rewriting pushed history is never allowed. Say what you rewrote.',
    '- prTitle must satisfy the repo\'s own PR-title rules (see AGENTS.md / CONTRIBUTING.md).',
    '',
    '## Issue' + (plural ? 's' : ''),

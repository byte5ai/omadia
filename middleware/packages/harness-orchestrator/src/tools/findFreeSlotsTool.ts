...
const FindFreeSlotsInputSchema = z.object({
   ...
   * - **Fremde Email** → dessen Kalender ("Teresita, such Termin bei Marcel"
   *   → hostEmail=marcel@byte5.de). Caller braucht `Calendars.Read.Shared`-
...
export const findFreeSlotsToolSpec = {
  ...
  description:
    ...
    '\n' +
    '**Regeln:**\n' +
    '- Slot-Suche läuft gegen den Kalender des **Hosts** (Meeting-Organizers). Standard = Caller selbst. Nur wenn der Caller explizit im Auftrag einer anderen Person sucht ("such Termin bei Marcel", "bei der Geschäftsführung"), setz `hostEmail` auf dessen Email.\n' +
  ...
  input_schema: {
    ...
    properties: {
      hostEmail: {
        type: 'string',
        description:
          'Email/UPN des Meeting-Hosts (wessen Kalender die Slots liefert). Leer lassen wenn der Caller selbst der Host ist ("ich biete an"); setzen wenn der Caller im Auftrag einer anderen Person Slots sucht ("such bei Marcel Termin" → hostEmail=marcel@byte5.de).',
      },
      attendees: {
        type: 'array',
        minItems: 0,

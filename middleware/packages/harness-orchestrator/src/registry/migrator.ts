...
const LOCK_KEY = '_multi_orchestrator_migrations';

/**
 * How long a replica waits for the migration lock before giving up. This
 * migrator runs inside `activate()`, which `ToolPluginRuntime` hard-caps at
 * 10s, so the wait has to be bounded well inside that — hence
 * `pg_try_advisory_lock` in a polling loop rather than the blocking
 * `pg_advisory_lock`, which would make a concurrent boot a deterministic
 * activation timeout. Exported so the budget can be asserted in a test
 * instead of trusted in a comment.
 */
export const MULTI_ORCH_MIGRATION_LOCK_WAIT_MS = 2_000;
 ...
 * line for line so the migrators stay diff-comparable. Bookkeeping table
 ...
 * lock, taken only after the ledger says there is work to do — the
 * steady-state boot takes no lock at all.
 */
export async function runMultiOrchestratorMigrations(
  ...
  try {
    ...
    let pending = await pendingMigrations(client, migrationsDir);
    if (pending.length === 0) return;

    const deadline = Date.now() + MULTI_ORCH_MIGRATION_LOCK_WAIT_MS;
    for (;;) {
    ...
    if (!lockHeld) {
      ...
      pending = await pendingMigrations(client, migrationsDir);
      if (pending.length === 0) {
        log(
          '[multi-orchestrator] migrations applied by another replica while waiting',
        );
        return;
      }
      ...
      throw new Error(
        `[multi-orchestrator] timed out after ${String(MULTI_ORCH_MIGRATION_LOCK_WAIT_MS)}ms waiting for the ${LOCK_KEY} advisory lock; ` +
...
function sleep(ms: number): Promise<void> {
  // `done`, not `resolve` — `resolve` is already the path import above.

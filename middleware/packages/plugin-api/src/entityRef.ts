 ...
 * anchor ("<system>://<model>/<id>") that a future knowledge-graph ingest
 * can lift without LLM guesswork.
 */
export interface EntityRef {
  /**
   * Source-system namespace — free-form string so plugins can stage their
   * own namespaces (e.g. `'personal-notes'`, `'<your-integration>'`) into
   * the same EntityRef shape. See `EntityIngest.system` for the
   * plugin-side ingest contract.
   */
  system: string;

 ...
 * (4400/4401 belong to the knowledge-graph embedding registry and the
 * stale-vector clear); the second key is `hashtext(<ledger table>)`, so each
 * subsystem serialises against its own replicas only and never against a
 * different subsystem's migrations.
 */
const LOCK_NS_MIGRATIONS = 4_410;
const LOCK_KEY = '_auth_migrations';

/**
 * How long a replica waits for the migration lock before giving up.
 *
 * Three of the eight migrators run inside a plugin `activate()`, which
 * `ToolPluginRuntime` hard-caps at 10s, and the knowledge-graph plugin can
 * already have spent 6s of that budget in `waitForPostgres` before its
 * migrator is even called. 2s therefore has to be the ceiling for all eight —
 * the tightest consumer sets the bound for everyone, and the value is exported
 * so the budget can be asserted in a test instead of trusted in a comment.
 */
export const AUTH_MIGRATION_LOCK_WAIT_MS = 2_000;
 ...
 * loser's boot fails). So the apply loop runs under a session-scoped advisory
 * lock, taken with `pg_try_advisory_lock` (never the blocking variant — an
 * unbounded server-side wait inside a 10s `activate()` would turn a rare race
 * into a deterministic boot failure) and only after the ledger says there is
 * work to do, so the steady-state boot takes no lock at all.
 *
 * Mirrors `runGraphMigrations` and `runRoutineMigrations` line for line so
 * the migrators stay diff-comparable.
 */
export async function runAuthMigrations(
  ...
  try {
    ...
    const deadline = Date.now() + AUTH_MIGRATION_LOCK_WAIT_MS;
    for (;;) {
    ...
    if (!lockHeld) {
      ...
      if (pending.length === 0) {
        log('[auth] migrations applied by another replica while waiting');
        return;
      }
      ...
      throw new Error(
        `[auth] timed out after ${String(AUTH_MIGRATION_LOCK_WAIT_MS)}ms waiting for the ${LOCK_KEY} advisory lock; ` +
 ...
 * whether it was taken. The boolean is the whole point: the previous attempt
 * could not distinguish "acquired" from "someone else holds it", which is what
 * made its release path inert.
 */

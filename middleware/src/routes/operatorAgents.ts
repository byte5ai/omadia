...
export interface OperatorAgentsRouterOptions {
  ...
  /** #914 — the agent identity store (migration 0051). Late-bound like the
  ...
  function agentIdentity(res: Response): OperatorAgentIdentityDeps | undefined {
    ...
    if (!deps) {
      res.status(503).json({
        ...
        message:
          'the agent identity store is not wired — it registers once DATABASE_URL is set and migration 0051 has been applied.',
      });
      return undefined;
    }
    return deps;

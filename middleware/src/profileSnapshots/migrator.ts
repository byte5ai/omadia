...
const LOCK_KEY = '_profile_snapshot_migrations';

/**
 * How long a replica waits for the migration lock before giving up. Held at
 * the same 2s ceiling as the migrators that run inside a plugin `activate()`
 * (which `ToolPluginRuntime` hard-caps at 10s) so the bound is one number
 * across all eight rather than eight numbers to reason about. Exported so the
 * budget can be asserted in a test instead of trusted in a comment.
 */
export const PROFILE_SNAPSHOT_MIGRATION_LOCK_WAIT_MS = 2_000;
...
export async function runProfileSnapshotMigrations(
  ...
  try {
    ...
    const deadline = Date.now() + PROFILE_SNAPSHOT_MIGRATION_LOCK_WAIT_MS;
    for (;;) {
    ...
    if (!lockHeld) {
      ...
      if (pending.length === 0) {
        log(
          '[profile-snapshot] migrations applied by another replica while waiting',
        );
        return;
      }
      ...
      throw new Error(
        `[profile-snapshot] timed out after ${String(PROFILE_SNAPSHOT_MIGRATION_LOCK_WAIT_MS)}ms waiting for the ${LOCK_KEY} advisory lock; ` +

...
export class DynamicAgentRuntime {
  ...
  async activate(agentId: string): Promise<DomainTool | null> {
    ...
    // the first inside LocalSubAgent.toolsByName.
    const subtoolIds = handle.toolkit.tools.map((t) => t.id);
...
function bridgeTool(td: UploadedToolkit['tools'][number]): LocalSubAgentTool {

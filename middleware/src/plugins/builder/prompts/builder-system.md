...
- Mit **`read_slot`** den aktuellen Source eines bereits gefüllten Slots
  abrufen — Output ist 1:1 was im Draft steht. Nutze das vor dem Erweitern
  eines Slots in resumed Sessions (im Spec-Header siehst du nur die *Keys*
  gefüllter Slots, nicht deren Inhalt), für Cross-Slot-Konsistenzchecks
  (z.B. ob ein im `activate-body` aufgerufenes Symbol wirklich im
  `toolkit-impl` definiert ist) und für Self-Debugging-Sessions, in denen
  der User dich nach dem geschriebenen Code fragt. Pure Read, kein
  tsc-Gate.

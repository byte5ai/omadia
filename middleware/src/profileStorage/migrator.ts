...
const LOCK_KEY = '_profile_storage_migrations';
 ...
 */
export const PROFILE_STORAGE_MIGRATION_LOCK_WAIT_MS = 2_000;
...
export async function runProfileStorageMigrations(
  ...
  try {
    ...
    const deadline = Date.now() + PROFILE_STORAGE_MIGRATION_LOCK_WAIT_MS;
    for (;;) {
    ...
    if (!lockHeld) {
      ...
      if (pending.length === 0) {
        log(
          '[profile-storage] migrations applied by another replica while waiting',
        );
        return;
      }
      ...
      throw new Error(
        `[profile-storage] timed out after ${String(PROFILE_STORAGE_MIGRATION_LOCK_WAIT_MS)}ms waiting for the ${LOCK_KEY} advisory lock; ` +

 ...
 * `ctx.config.get(...)` is declared in the plugin's manifest. It scans the
 * package's `src/` so it also covers keys added in future files.
 */
 ...
 *  in their own manifest (the kernel adds them to every plugin's schema). */
const KERNEL_INJECTED_KEYS = new Set<string>(['_privacy_mode']);
...
/** Every distinct key read via `ctx.config.get(...)` across the given sources. */
function configKeysReadIn(files: readonly string[]): Set<string> {
  // Matches `.config.get('key')` and `.config.get<T>('key')`, single or double
  // quotes. Deliberately narrow: only literal-string keys are checked (a dynamic
  // key can't be validated statically anyway).

...
import {
  ...
  teamsProvisioningLastErrorProducers as produce,
...
import { teamsLastErrorDetail } from '../src/routes/operatorAgents.js';
...
describe('classifyTeamsProvisioningError — round trip with the producers', () => {
  it('classifies the consent sentence and recovers the missing scopes', () => {
    const raw = produce.consentMissingDetail([
  ...
  it('classifies the ARM sentence and recovers the missing setup fields', () => {
    const raw = produce.armNotConfiguredDetail([
      'azureSubscriptionId',
      'azureResourceGroup',
    ]);
  ...
  it('keeps an empty list empty instead of inventing a placeholder scope', () => {
    const detail = classifyTeamsProvisioningError(produce.consentMissingDetail([]));
  ...
  it('classifies an exhausted throttle and recovers the Retry-After hint', () => {
    const raw = produce.retriesExhaustedDetail(new Error('429 from Graph'), 3, {
  ...
  it('classifies a throttle without a Retry-After header as throttled, hint omitted', () => {
    const raw = produce.retriesExhaustedDetail(new Error('429 from ARM'), 2, {});
  ...
  it('classifies an exhausted non-throttle error as unknown, message preserved', () => {
    const raw = produce.retriesExhaustedDetail(new Error('socket hang up'), 2, undefined);

    const detail = classifyTeamsProvisioningError(raw);

    assert.equal(detail?.code, 'unknown');
    assert.equal(detail?.raw, 'socket hang up (gave up after 2 attempts)');
  });

  it('is total: an unrecognized or empty sentence never throws', () => {
    assert.equal(classifyTeamsProvisioningError(null), null);
    assert.equal(classifyTeamsProvisioningError(undefined), null);
    assert.equal(classifyTeamsProvisioningError('   '), null);
...
describe('teamsLastErrorDetail — operator wire projection', () => {
  it('emits snake_case and omits the members that do not apply', () => {
    const projected = teamsLastErrorDetail(
      produce.armNotConfiguredDetail(['azureSubscriptionId']),
    );
    ...
    assert.ok(!('retry_after_seconds' in (projected as object)));
  });

  it('renames retryAfterSeconds to retry_after_seconds for the wire', () => {
    const projected = teamsLastErrorDetail(
      produce.retriesExhaustedDetail(new Error('429'), 3, { retryAfterSeconds: 7 }),
    );

    assert.equal(projected?.retry_after_seconds, 7);
  });

  it('null last_error projects to null — a healthy row carries no detail', () => {
    assert.equal(teamsLastErrorDetail(null), null);
  });
});

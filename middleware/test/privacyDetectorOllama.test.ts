...
describe('nerPrompt · message scaffolding (Slice 3.2)', () => {
  it('builds [system, fewShot×N alternating, user]', () => {
    const msgs = buildNerMessages('Hi Marcel.');
    ...
    assert.equal(last?.content, 'Hi Marcel.');
  });
...
describe('createOllamaNerDetector · adapter contract (Slice 3.2)', () => {
  it('detector id is surfaced verbatim on every hit', async () => {
    const detector = createOllamaNerDetector({
      client: stubClient(
        () =>
          '{"hits":[{"type":"pii.name","value":"Marcel","start":3,"end":9,"confidence":0.9}]}',
      ),
      model: 'llama3.2:3b',
    ...
    const { hits } = await detector.detect('Hi Marcel hi.');
  ...
  it('returns [] (fail-open) when the chat client throws', async () => {
    ...
    const { hits } = await detector.detect('Hi Marcel.');
    assert.deepEqual(hits, []);
  });
  ...
  it('re-anchors hits via indexOf when the model returns wrong-but-shape-valid offsets', async () => {
    ...
    const detector = createOllamaNerDetector({
      client: stubClient(
        () =>
          `{"hits":[{"type":"pii.name","value":"Marcel Wege","start":0,"end":5,"confidence":0.95}]}`,
      ),
      model: 'llama3.2:3b',
      detectorId: 'ollama:llama3.2:3b',
      maxInputChars: 8000,
    ...
    assert.equal(text.slice(hit.span[0], hit.span[1]), 'Marcel Wege');
    assert.equal(hit.span[0], text.indexOf('Marcel Wege'));
  });
  ...
  it('disambiguates two identical values to distinct spans via cursor advance', async () => {
    const text = 'Marcel hat Marcel verloren.';
    const detector = createOllamaNerDetector({
      client: stubClient(
        () =>
          `{"hits":[` +
          `{"type":"pii.name","value":"Marcel","start":0,"end":6,"confidence":0.9},` +
          `{"type":"pii.name","value":"Marcel","start":11,"end":17,"confidence":0.9}` +
          `]}`,
      ),
      model: 'llama3.2:3b',
      detectorId: 'ollama:llama3.2:3b',

...
- [x] Merged `origin/main` (already current), committed (`232739b`), pushed `feat/336-minimal-onboarding-stack`, opened **PR #339** linking #336. Awaiting user merge to main (self-merge blocked).
- [x] CI fix: the only red checks were `audit (high+critical)` from a **pre-existing** `undici` advisory (published 2026-06-18 14:28 UTC; main green only because its audit ran earlier — same lockfile fails today). Not caused by #336 (no dep changes in the original diff). Fix folded into this PR per user: middleware `undici ^8.3.0→^8.5.0` (direct, patched), web-ui `overrides.undici ^7.28.0` (transitive). Both audit gates pass locally; `Agent`/`fetch` exports verified stable in 8.5.0; the cold-worktree `tsc` TS2307 errors are unrelated (unbuilt workspace `dist/`, CI builds first).
